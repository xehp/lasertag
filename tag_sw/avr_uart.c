...
//A counter incremented every time there is some error in uart.
...
void uart_putchar(char ch)
{
	...
	while(uartTxHead==tmp)
	{
		...
		avr_wtd_reset_and_sleep(); // set CPU idle to save power during this busy wait.
	}
...
void uart_init(void)
{
	...
	// NOTE that ubrr must not be more than 4095, but that only happens at very low baudrates.
...
void uart_print_PL(const char *pgm_addr)
{
	char c;
	while ((c = pgm_read_byte(pgm_addr++)))
	{
...
void uart_print_hex64(uint64_t i)
{
	if (uart_get_free_space_in_write_buffer() > (UART_TX_BUFFER_SIZE/4))
	{
		uart_print_hex4((i>>60)&0xf);
		uart_print_hex4((i>>56)&0xf);
		uart_print_hex4((i>>52)&0xf);
		uart_print_hex4((i>>48)&0xf);
		uart_print_hex4((i>>44)&0xf);
		uart_print_hex4((i>>40)&0xf);
		uart_print_hex4((i>>36)&0xf);
		uart_print_hex4((i>>32)&0xf);
		uart_print_hex4((i>>28)&0xf);
		uart_print_hex4((i>>24)&0xf);
		uart_print_hex4((i>>20)&0xf);
		uart_print_hex4((i>>16)&0xf);
...
void uart_print_hex(const uint8_t *data, uint8_t sizeof_data)
{
	if (uart_get_free_space_in_write_buffer() > (UART_TX_BUFFER_SIZE/4))
	{
		for (uint8_t i=0; i<sizeof_data; ++i)
		{
