...
Parameters:
  tone is given as divider.
  volume shall be given in percent of max. (100% will give 50% duty cycle)
*/
void avr_tmr1_pwm_on(uint16_t tone, uint8_t volume_percent)
{
	...
	if ((tone != 0) && (volume_percent!=0))
	{
		...
		// Fast PWM mode using ICR1 as TOP
		// TODO should we use phase correct PWM? As now the A will be active
		// in beginning of cycle and B in the end. All "pause" is in the middle.
		...
		*/
		...
		const uint32_t v = volume_percent;
		const uint32_t f = 2*100;
		const uint32_t d = (v*tone)/f;
